...
import { deleteMaintenance } from '../services/call-api';
    ...
    return [
      {
        ...
        type: 'success',
        content: 'Thêm mới',
        onClick: () => window._$g.rdr(`/maintenance/add`),
      },
      {
        icon: 'fi fi-rr-edit',
        color: 'blue',
        ...
        onClick: (_, d) =>
          dispatch(
            showConfirmModal(
              ...
              async () => {
                await deleteMaintenance([parseInt(_?.id)]);
                onRefresh();
              },
            ),
          ),
      ...
      handleBulkAction={(e) => {
        dispatch(
          showConfirmModal(
            ['Bạn có thực sự muốn xóa?', 'Bạn sẽ mất dữ liệu này và các dữ liệu liên quan.'],
            async () => {
              await deleteMaintenance(e?.map((val) => parseInt(val?.id)));
              onRefresh();
            },
          ),
        );
