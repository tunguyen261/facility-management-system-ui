...
const InformationMulti = ({ disabled, title }) => {
  ...
  const loadUserOpts = useCallback(() => {
    getListUser(params)
      .then((res) => {
        const list = res?.data;
        const _list = list.filter((i) => i.role === 3);
        setUserList(_list);
      })
      .catch((err) => {
        showToast.error(err?.message ?? 'Có lỗi xảy ra');
        ...
        return (
              ...
              <div className='ca_row'>
                ...
                <FormItem className='ca_col_4' label='Phạm vi thực hiện' isRequired disabled={disabled}>
                  <FormSelect
                    ...
                    disabled={disabled}
                    onChange={(value) => {
                      setValue(`form_mutil.${idx}.is_internal`, value);
                      loadUserOpts();
                    }}
                  />
                </FormItem>
                ...
                <FormItem
                  ...
                  <FormSelect
                    ...
                    disabled={disabled}
                    onChange={(value) => {
                      setValue(`form_mutil.${idx}.type_id`, value);
                      setValue(`form_mutil.${idx}.asset_id`, null);
                      loadAssetOpts(value);
                    }}
                  />
                ...
                <FormItem
                  ...
                  isRequired
                  disabled={
                    disabled || !watch(`form_mutil.${idx}.category_id`) || !watch(`form_mutil.${idx}.is_internal`)
                  }>
                  <FormSelect
          ...
          <FormDatePicker
            ...
            />
              ...
              <Attachments disabled={disabled} keyObject={`form_mutil.${idx}.related_files`} />
              <div className='ca_row'>
