...
import { deleteRepairation } from '../services/call-api';
...
const columns = useMemo(
    ...
    },
    {
        ...
        formatter: (d) => <p style={{ cursor: 'pointer'}} onClick={()=>{window._$g.rdr(`/asset/detail/${d.asset.id}`)}} >{d?.asset?.asset_name + ' - ' + d?.asset?.asset_code}</p>,
    },
    {
        header: 'Mô tả',
        classNameHeader: 'ca_text_center',
    ...
    //   },
      ...
      },
      {
        ...
        accessor: 'created_at',
        formatter: (p) => 
            p.is_internal ? <p>Thực hiện nội bộ</p> :  <p>Thực hiện bên ngoài</p>
        ,
      },
    ],
    [],
...
const actions = useMemo(() => {
    return [
    {
        ...
        type: 'success',
        content: 'Thêm mới',
        onClick: () => window._$g.rdr(`/repairation/add`),
    },
    {
        icon: 'fi fi-rr-edit',
        color: 'blue',
        disabled: (p) => p?.status !== REQUEST_STATUS.NotStart,
        onClick: (p) => window._$g.rdr(`/repairation/edit/${p?.id}`),
    },
    {
        icon: 'fi fi-rr-eye',
        color: 'green',
        onClick: (p) => window._$g.rdr(`/repairation/detail/${p?.id}`),
    },
    {
        icon: 'fi fi-rr-trash',
        color: 'red',
        disabled: (p) => p?.status === REQUEST_STATUS.Done,
        onClick: (_, d) =>
          dispatch(
            showConfirmModal(
              ...
              async () => {
                try{
                  await deleteRepairation(_?.id);
                  onRefresh();
                }catch (ex){
                  showToast.error(ex?.message || 'Có lỗi xảy ra')
                }
              },
            ),
          ),
