...
const Attachments = ({ disabled, onRefesh, title }) => {
  ...
  const handleRemoveFile = (idx, e) => {
    e.preventDefault();
    if (!disabled) {
      let _files = watch('related_files');
      ...
      if (file_module_id && file_id) {
        ...
        _files.splice(idx, 1);
        methods.setValue('related_files', _files || []);
      }
    }
  };
  ...
  const handleUploadFile = async (e) => {
    ...
    methods.setValue('related_files', _files.concat(attachmentList));
  };
  ...
  return (
    <div className='ca_col_12'>
      ...
      {watch('related_files') && watch('related_files').length > 0
        ? watch('related_files').map((item, idx) => {
            return (
