...
const Information = ({ disabled, title }) => {
  ...
  const loadAssetOpts = useCallback(() => {
    getAssetList({ room_id: watch('room_id') })
      .then((res) => {
  ...
  useEffect(loadAssetOpts, [loadAssetOpts, watch('room_id')]);
    ...
    <CAAccordion title={title}>
      <div className='ca_col_12'>
        <div className='ca_row'>
          ...
          <FormItem className='ca_col_4' label='Phòng báo cáo' isRequired disabled={disabled}>
            <FormSelect list={mapDataOptions4Select(roomList, 'id', 'room_name')} field='room_id' disabled={disabled} />
          </FormItem>
          <FormItem className='ca_col_4' label='Thiết bị' isRequired disabled={disabled || !watch('room_id')}>
            <FormSelect
          ...
          <FormItem className='ca_col_4' label='Trạng thái' isRequired disabled={true}>
            <FormSelect
          ...
          {isView && (
            <FormItem className='ca_col_4' label='Ngày hoàn thành' isRequired disabled={disabled || true}>
              <FormDatePicker disabled={disabled} type='text' field='completion_date' placeholder='Ngày hoàn thành' />
            </FormItem>
          )}
          {isView && (
